...
export default function TelemetryStrip({ activeZone, riskPercent, liveTelemetry, isLiveInternet = true }: TelemetryStripProps) {
  ...
  return (
    <div className="w-full bg-transparent px-4 sm:px-6 lg:px-8 py-2.5 text-slate-900 font-sans">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3 max-w-[1800px] mx-auto">
        ...
        >
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-slate-900 text-white flex items-center justify-center group-hover:bg-black transition shrink-0 shadow-xs">
              <Compass className="w-5 h-5 text-white" />
            </div>
            <div>
              <h2 className="text-xs sm:text-sm font-extrabold text-slate-950 flex items-center gap-2">
                <span>Live Flood Radar Map</span>
                <span className="text-[10px] font-black px-2 py-0.5 rounded-full bg-emerald-50 text-emerald-800 border border-emerald-300 uppercase tracking-wider flex items-center gap-1.5">
          ...
          <ArrowRight className="w-4 h-4 text-slate-400 group-hover:text-slate-900 group-hover:translate-x-1 transition shrink-0 ml-2" />
        </Link>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xs sm:text-sm font-extrabold text-slate-950 flex items-center gap-2">
                ...
                <span className="text-[10px] font-black px-2 py-0.5 rounded-full bg-red-50 text-red-700 border border-red-300 uppercase tracking-wider flex items-center gap-1.5">
        ...
        <div className="glass-card rounded-2xl p-3 sm:px-4 sm:py-3 transition flex items-center justify-between min-h-[72px]">
          <div className="flex-1 mr-2 space-y-0.5">
            ...
            <div className="text-base sm:text-lg font-black font-mono text-slate-950 flex items-baseline gap-2">
              <span>{telemetry.slope_deg.toFixed(1)}°</span>
              <span className="text-[11px] font-medium text-slate-500 font-sans">
                Incline &bull; <span className="font-mono font-bold text-slate-800">{telemetry.soil_moisture_pct.toFixed(0)}%</span> Moisture
              </span>
            </div>
