...
export default function DisasterLocationStream() {
  ...
  return (
    <div className="rounded-2xl border border-red-200 bg-white shadow-lg overflow-hidden font-sans">
      {/* Header Banner */}
      <div className="bg-gradient-to-r from-red-600 via-rose-600 to-red-700 px-5 py-4 text-white flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          <span className="px-3 py-1 rounded-lg bg-black/25 text-white text-xs font-semibold border border-white/20 flex items-center gap-1.5">
      ...
      <div className="p-5 space-y-5">
        {latest ? (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            ...
            <div className="lg:col-span-1 rounded-xl bg-red-50/70 border border-red-100 p-4 space-y-3">
              ...
              <div className="flex items-center gap-3 pt-1">
                <div className="w-12 h-12 rounded-xl bg-white shadow-xs border border-red-200 flex items-center justify-center">
                  <Smartphone className="w-6 h-6 text-red-600" />
                </div>
                <div>
                  <h4 className="font-bold text-slate-900 text-base">{latest.phone_model || "Vivo V2437"}</h4>
                  <p className="text-xs text-slate-500 font-mono">UUID: {latest.device_uuid.slice(0, 14)}...</p>
                </div>
              </div>

              {/* Exact Coordinates */}
              <div className="bg-white rounded-lg p-3 border border-red-100 space-y-1">
                <div className="flex items-center justify-between text-xs">
                  <span className="text-slate-500 font-medium flex items-center gap-1">
                    <MapPin className="w-3.5 h-3.5 text-red-500" /> Live GPS:
                  </span>
              ...
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="w-full py-2 px-3 rounded-lg bg-red-600 hover:bg-red-700 text-white text-xs font-bold flex items-center justify-center gap-1.5 transition shadow-xs"
              >
                ...
                {breadcrumbs.slice(0, 15).map((point, index) => (
                  <div
                    ...
                    className={`px-4 py-2 flex items-center justify-between transition ${
                      index === 0 ? "bg-red-50/50 font-semibold" : "hover:bg-slate-50"
                    }`}
                  >
                    <div className="flex items-center gap-2">
                      ...
                      {index === 0 && (
                        <span className="px-1.5 py-0.5 rounded-sm bg-red-600 text-white text-[9px] font-bold">
                          LATEST
                        </span>
                      )}
                    </div>
          ...
          <div className="text-center py-8 text-slate-500 space-y-2">
            ...
            <p className="text-sm font-semibold text-slate-700">Waiting for 5-Second Disaster Telemetry Stream...</p>
